...
def get_state(obs,index_list_or,index_list_ex, v_nom_list) :
    ...
    state = np.concatenate((state,power_data))
...
env_name = "l2rpn_2019" 
reward_function = 'V_complexe' # 'V_MSE' ou 'V_complexe'
...
input_length = bus_lentgh + len(obs.rho) + len(obs.gen_p) + len(obs.gen_q) + len(obs.load_p) + len(obs.load_q)#type: ignore
