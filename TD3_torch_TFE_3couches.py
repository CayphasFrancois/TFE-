...
class CriticNetwork (nn.Module) : # objectif du critic : estimer Q
    def __init__ (self,beta,input_dims,fc1_dims,fc2_dims,fc3_dims,n_actions,name,chkpt_dir='C:/TFE-Code/TFE_Grid2Op/checkpoint_TD3_TFE'):
        ...
        self.fc2 = nn.Linear(self.fc1_dims,self.fc2_dims) # deuxième couche (état transformé)
        f2 = 1/np.sqrt(self.fc2.weight.data.size()[0])
        T.nn.init.uniform_(self.fc2.weight.data,-f2,f2) 
        T.nn.init.uniform_(self.fc2.bias.data,-f2,f2)
        self.bn2= nn.LayerNorm (self.fc2_dims)
        
        self.fc3 = nn.Linear(self.fc2_dims,self.fc3_dims) # troisième couche (état transformé)
        ...
        self.action_value = nn.Linear(self.n_actions , fc3_dims) # projection de l'action dans le même espace que fc3
        f3 = 0.003
        ...
        self.fc4 = nn.Linear(*self.input_dims, self.fc1_dims) # type: ignore # première couche où on met que l'état
        ...
        self.fc5 = nn.Linear(self.fc1_dims, self.fc2_dims) # deuxième couche (état transformé)
        ...
        self.fc6 = nn.Linear(self.fc2_dims,self.fc3_dims) # troisième couche (état transformé)
        ...
        self.bn6= nn.LayerNorm (self.fc3_dims)

        self.action_value2 = nn.Linear(self.n_actions , fc3_dims) # projection de l'action dans le même espace que fc3
        f3 = 0.003
        self.q2 = nn.Linear(self.fc3_dims, 1)
    ...
    def forward (self,state,action) : 
        ...
        state_value1 = self.fc2(state_value1)
        state_value1 = self.bn2 (state_value1)
        state_value1 = F.relu(state_value1)
        state_value1 = self.fc3(state_value1)
        state_value1 = self.bn5(state_value1)
        state_value1 = F.relu(state_value1)
        ...
        state_value2 = self.fc5(state_value2)
        state_value2 = self.bn4 (state_value2)
        state_value2 = F.relu(state_value2)
        state_value2 = self.fc6(state_value2)
        state_value2 = self.bn6(state_value2)
        state_value2 = F.relu(state_value2)
    ...
    def q1_only(self, state, action):
        ...
        # Pour calculer la valeur de la perte pour l'Actor µ (que Q1)

        s1 = F.relu(self.bn1(self.fc1(state)))
        s1 = F.relu(self.bn2(self.fc2(s1)))
        s1 = F.relu(self.bn5(self.fc3(s1)))
...
class ActorNetwork(nn.Module) : 
    def __init__ (self,alpha,input_dims,fc1_dims,fc2_dims,fc3_dims,n_actions,name,chkpt_dir='C:/TFE-Code/TFE_Grid2Op/checkpoint_TD3_TFE') :
...
class Agent(object) : 
    def __init__(self,alpha,beta,input_dims,tau,env,gamma=0.99, n_actions=2, max_size= 1000000, layer1_size=400,layer2_size=300,layer3_size = 200, batch_size=64,exploration_noise = 0.1, policy_noise=0.2,noise_clip=0.5,update_actor_iter=2,learn_step_cntr=0,seed=42,filename='_') :
        ...
        self.actor = ActorNetwork(alpha, input_dims, layer1_size, layer2_size,layer3_size, n_actions = n_actions, name='Actor',chkpt_dir=filename)

        self.target_actor = ActorNetwork(alpha, input_dims, layer1_size, layer2_size,layer3_size, n_actions = n_actions, name='Target_Actor',chkpt_dir=filename)

        self.critic = CriticNetwork (beta, input_dims, layer1_size, layer2_size,layer3_size, n_actions = n_actions,name= 'Critic',chkpt_dir=filename)

        self.target_critic = CriticNetwork (beta, input_dims, layer1_size, layer2_size,layer3_size, n_actions = n_actions,name= 'Target_Critic',chkpt_dir=filename)
